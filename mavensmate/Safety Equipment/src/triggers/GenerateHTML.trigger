trigger GenerateHTML on Opportunity (before update) {
	...
	List<String> listSlingLines = new List<String>();
	List<String> listHarnesses = new List<String>();
	List<String> listETKs = new List<String>();
	List<String> listHarnessRacks = new List<String>();
	List<String> listEntranceBarriers = new List<String>();
	 ...
	 for(Opportunity opp : trigger.new){ 
	 	...
	 	List<Project_Module__c> projectsModules = [
	 		SELECT 
	 	 	...
	 	 	for(Project_Module__c p: projectsModules){
 				 /* Update all of the values if one of the modules is a Sky Trail */

	 	 		if(p.Module_Type__c=='Sky Trail'){
	 	 		String header;
	 	 			header = '<b>'+  p.Module_Type__c + '&reg ' + p.Course_Model__c +  ' - ' + p.Number_of_Poles__c + ' Pole, ' + p.Number_of_Levels__c + ' Level';
	 	 			String inclines = '';
	 	 			String elements;
	  				Integer elementCount = 0;
	 	 			elementCount =  Integer.valueOf(p.Number_One_Way_Inclines__c) + Integer.valueOf(p.Number_One_Way_Inclines__c)  + Integer.valueOf(p.Number_of_Elements__c);
	 	 			...
	 	 			if(p.Course_Model__c=='Sky Tykes'){
	 	 					...
	 	 					if(p.Sky_Tykes_StandAlone__c == true){ /*If it is a standalone the language is a little different */
	 	 						listSkyTykesOverview.add(header + ' Overview </b>' + '</br>The course will be built on ' + 
	 	 							...
	 	 							listSlingLines.add(header +  '</b></br>' + p.Participant_Sling_Lines__c + ' participant sling lines are included in the final purchase price.  Operator sling lines are not required to operate this course.');

	 	 						}else{/*If it is a attached the language is a little different */
	 	 							listSkyTykesOverview.add('<b>'+ 'Attached ' + p.Number_of_Poles__c + ' Pole ' + p.Course_Model__c + ' Overview </b>' + '</br>The course will be built on ' + p.Number_of_Poles__c + 
	 	 				...
	 	 				/* Not a Sky Tykes so must be another model of the Sky Trail */
	 						listOverview.add('<b>'+ p.Module_Type__c + '&reg ' + p.Course_Model__c + ' ' + p.Number_of_Poles__c + ' Pole ' +  p.Number_of_Levels__c + ' Level Overview</b></br>The course will be built on ' + 
	 						...
	 						listSlingLines.add(header + '</b></br>' + p.Operator_Sling_Lines__c + ' orange operator and ' + p.Participant_Sling_Lines__c + ' participant sling lines are included in the final purchase price.');

	 				}
	  				
	  				listProjectTypes.add('<b>'+p.Module_Type__c + '&reg '  + p.Course_Model__c +' - ' + p.Number_of_Poles__c + ' Pole, ' + p.Number_of_Levels__c + ' Level ' + ' ??incline</b>');	
	  				listProjectElements.add(elements);	  				
	  				listHarnesses.add(header + '</b></br> Harnesses include ' + 'X-Small: '+ p.X_Small_Quantity__c +', Small: ' + p.Small__c+ ', Medium: ' + p.Medium__c + ', Large: ' + p.Large__c+'.') ;
	  				listETKs.add(header + '</b></br>' + p.Number_of_ETKs__c +' are included in the final purchase price.');	
	  				listHarnessRacks.add(header + '</b></br>' + p.Number_Of_Harness_Racks__c + ' harness racks are included in the final purchase price.');
					...
					listOptions.add(options);
	 	 			...
	 	 			//in some cases the wording is slightly different for stand-alone vs. parallel Sky Rails
	 	 			if(p.StandAlone__c==false){
	 	 				skyRailHeader = '<b>Parallel with Course  ' + p.Module_Type__c + ' ~ ' + p.Sky_Rail_Length__c.round() + ' in length';
	 	 				listSlingLines.add(skyRailHeader + '.</b></br>Uses the operator and participant sling lines included in the final purchase price.');
	 	 				listHarnesses.add(skyRailHeader +'. </b></br>Uses the operator and participant harnesses included in the final purchase price.');	 
	 	 			}else{
	 	 				skyRailHeader = '<b>Stand-Alone ' + p.Module_Type__c + ' ~ ' + p.Sky_Rail_Length__c.round() + ' in length';
	 	 				listSlingLines.add(skyRailHeader + '. </b></br>' + p.Operator_Sling_Lines__c + ' orange operator and ' + p.Participant_Sling_Lines__c + ' participant sling lines are included in the final purchase price.');
	 	 				listHarnesses.add(skyRailHeader + '. </b></br>Harnesses include ' + 'X-Small: '+ p.X_Small_Quantity__c +', Small: ' + p.Small__c+ ', Medium: ' + p.Medium__c + ', Large: ' + p.Large__c+'.') ;	 
	 					...
	 					listETKs.add(skyRailHeader + '.</b></br>' + p.Number_of_ETKs__c +' are included with the course');
	 					listHarnessRacks.add(skyRailHeader + '.</b></br>' + p.Number_Of_Harness_Racks__c + ' harness racks are included in the final purchase price.');
	 				...
	 				listSlingLines.add('<b>' + p.Module_Type__c + ' ~ ' + p.Zip_Line_Distance_s__c + ' Feet'+ '</b></br>' +  p.CTS_Zip_Trolleys__c + ' trolleys and sling lines are inlcuded in the final purchase price.');
	 				listHarnesses.add('<b>' + p.Module_Type__c + ' ~ ' + p.Zip_Line_Distance_s__c + ' Feet'+ '</b></br>Harnesses include ' + 'X-Small: '+ p.X_Small_Quantity__c +', Small: ' + p.Small__c+ ', Medium: ' + p.Medium__c + ', Large: ' + p.Large__c+'.') ;	 
	 				listETKs.add('<b>' + p.Module_Type__c + ' ~ ' + p.Zip_Line_Distance_s__c + ' Feet'+ '</b></br>' + p.Number_of_ETKs__c +' are included with the course');
	 				listHarnessRacks.add('<b>' + p.Module_Type__c + ' ~ ' + p.Zip_Line_Distance_s__c + ' Feet'+ '</b></br>' + p.Number_Of_Harness_Racks__c + ' harness racks are included with the course');
		...
		private void renderHtmlFields(
										...
										List<String> projectSlingLines,
										List<String> projectHarnesses,
										List<String> projectETKs,
										List<String> projectHarnessRacks,
										List<String> projectEntranceBarriers,
										List<String> projectOptions
										){

			opp.htmlProjectTypes__c = '';
			...
			opp.htmlSlingLines__c = '<ul>';
			opp.htmlHarnesses__c = '<ul>';
			opp.htmlETKs__c = '<ul>';
			opp.htmlHarnessRacks__c = '<ul>';
			opp.htmlEntranceBarriers__c = '<ul>';
			opp.htmlOptions__c = '<ul>';

			for (String i :projectTypes) {opp.htmlProjectTypes__c = opp.htmlProjectTypes__c + '<li>' + i + '</li>';	}
			...
			for (String i :projectSlingLines) {opp.htmlSlingLines__c = opp.htmlSlingLines__c + '<li>' + i + '</li></br>';}
			
			for (String i :projectHarnesses) {opp.htmlHarnesses__c = opp.htmlHarnesses__c + '<li>' + i + '</li></br>';}
			
			for (String i :projectETKs) {opp.htmlETKs__c = opp.htmlETKs__c + '<li>' + i + '</li></br>';}
			
			for (String i :projectHarnessRacks) {opp.htmlHarnessRacks__c = opp.htmlHarnessRacks__c + '<li>' + i + '</li></br>';}
			...
			opp.htmlProjectTypes__c = opp.htmlProjectTypes__c +'</ul>';
			...
			opp.htmlSlingLines__c = opp.htmlSlingLines__c + '</ul>';
			opp.htmlHarnesses__c =  opp.htmlHarnesses__c +'</ul>';
			opp.htmlETKs__c = opp.htmlETKs__c + '</ul>';
			opp.htmlHarnessRacks__c =  opp.htmlHarnessRacks__c +'</ul>';
